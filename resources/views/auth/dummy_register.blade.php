...
<body>
    <div class="container">
        <h2>Daftar Akun Dummy</h2>
        @if($errors->any())
            <div class="error">{{ $errors->first() }}</div>
        @endif
        <form method="POST" action="{{ route('dummy.register') }}">
            @csrf
        ...
        <a href="{{ route('dummy.login') }}" class="link">Sudah punya akun? Masuk di sini</a>
    </div>
</body>
</html> 