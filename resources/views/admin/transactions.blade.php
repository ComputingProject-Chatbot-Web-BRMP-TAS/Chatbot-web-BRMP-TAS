                        ...
                        <form method="GET" action="{{ route('admin.transactions') }}" class="row g-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="payment_status" class="form-select">
                                    ...
                                    <option value="approved" {{ request('payment_status') == 'approved' ? 'selected' : '' }}>Dikonfirmasi</option>
                                    <option value="rejected" {{ request('payment_status') == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                                    <option value="no_payment" {{ request('payment_status') == 'no_payment' ? 'selected' : '' }}>Belum Bayar</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-4">
                                ...
                                <div>
                                    ...
                                    <a href="{{ route('admin.transactions') }}" class="btn btn-secondary">Reset</a>
                                </div>
                            </div>
                        </form>
                    </div>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <br>
                                            <small class="text-muted">{{ $transaction->transactionItems->count() }} item</small>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <br>
                                            <small class="text-muted">{{ $transaction->delivery_method ?? 'N/A' }}</small>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @if($latestPayment)
                                                ...
                                                @endif
                                                <br>
                                                <small class="text-muted">{{ $latestPayment->payment_date ? $latestPayment->payment_date->format('d/m/Y') : $latestPayment->created_at->format('d/m/Y') }}</small>
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <a href="{{ route('admin.transaction.detail', $transaction->transaction_id) }}" 
                                                ...
                                                @if($latestPayment && $latestPayment->payment_status == 'pending')
                                                    <a href="{{ route('admin.payment.show', $latestPayment->payment_id) }}" 
                                                       class="btn btn-sm btn-warning">
                                                        <i class="fas fa-credit-card"></i> Konfirmasi Pembayaran ke-{{ $transaction->payments->where('payment_id', '<=', $latestPayment->payment_id)->count() }}
                                                    </a>
                                                @endif
                                            </div>
                                        </td>
