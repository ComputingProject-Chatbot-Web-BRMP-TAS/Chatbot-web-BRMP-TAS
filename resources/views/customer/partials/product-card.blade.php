...
<div class="product-card" data-price="{{ $product->price_per_unit }}" data-stock="{{ $availableStock }}"
    ...
    @endif
    <div class="info"
        style="padding:12px 16px 16px 16px;flex:1;display:flex;flex-direction:column;justify-content:space-between;">
        <div>
            <div class="title" style="font-weight:500;font-size:15px;margin-bottom:6px;line-height:1.3;">
                {{ $product->product_name ?? 'Nama Produk Tidak Tersedia' }}</div>
            <div class="price" style="color:#388E3C;font-weight:bold;font-size:16px;margin-bottom:4px;">
                Rp{{ number_format($product->price_per_unit ?? 0, 0, ',', '.') }} / {{ $product->unit ?? 'Kg' }}</div>
            <div class="desc"
                style="font-size:12px;color:#757575;height:32px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-overflow:ellipsis;">
                {{ $product->description ?? 'Deskripsi tidak tersedia' }}</div>
        </div>
