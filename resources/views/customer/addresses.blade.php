...
@section('content')
    ...
    <div class="addresses-container">
        <button class="btn-green w-100" id="btnTambahAlamatBaru" data-bs-toggle="modal" data-bs-target="#modalTambahAlamat">+
            Tambah
            Alamat Baru</button>
                ...
                @foreach ($addresses as $address)
                    <div class="mb-3 position-relative">
                        <form method="POST" action="{{ route('addresses.setPrimary', $address) }}">
                            @csrf
                            <div class="card @if ($address->is_primary) address-card-primary @elseif(!$address->is_primary) address-card-clickable @endif"
                                style="border-radius:12px;box-shadow:0 1px 6px rgba(0,0,0,0.04);"
                                @if (!$address->is_primary) onclick="this.closest('form').submit();" @endif>
                                <div
                                    ...
                                    <div style="flex:1;">
                                        ...
                                        @endif
                                        <div class="fw-bold" style="font-size:1.1em;">{{ $address->label ?? 'Alamat' }}
                                        </div>
                                        <div style="color:#555;font-size:0.98em;word-wrap:break-word;">
                                            {{ $address->address }}</div>
                                        <div style="color:#888;font-size:0.93em;">({{ $address->latitude }},
                                            {{ $address->longitude }})</div>
                                        @if ($address->note)
                                            <div style="color:#888;font-size:0.93em;">Catatan: {{ $address->note }}</div>
                                        @endif
                                        <div style="color:#888;font-size:0.93em;">Penerima: {{ $address->recipient_name }}
                                            ({{ $address->recipient_phone }})
                                        </div>
                                        <div class="mt-2 d-flex gap-2 flex-wrap">
                                            <button type="button" class="btn btn-outline-primary btn-sm"
                                                onclick="editAlamat({{ $address->address_id }}, event)"
                                                data-id="{{ $address->address_id }}" data-label="{{ $address->label }}"
                                                data-address="{{ $address->address }}"
                                                data-latitude="{{ $address->latitude }}"
                                                data-longitude="{{ $address->longitude }}"
                                                data-note="{{ $address->note }}"
                                                data-recipient_name="{{ $address->recipient_name }}"
                                                data-recipient_phone="{{ $address->recipient_phone }}">Edit Alamat</button>
                                            @if (!$address->is_primary)
                                                <button type="button" class="btn btn-outline-danger btn-sm"
                                                    onclick="hapusAlamat{{ $address->address_id }}(event)">Hapus</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <form id="formHapusAlamat{{ $address->address_id }}" method="POST"
                            action="{{ route('addresses.destroy', $address) }}" style="display:none;">
                            @csrf
                            @method('DELETE')
                        </form>
                        <script>
        ...
        @else
            <div class="address-empty">
                <img src="https://cdn-icons-png.flaticon.com/512/4076/4076549.png" alt="Alamat kosong"
                    class="address-empty-img">
        ...
        document.addEventListener('DOMContentLoaded', function() {
                    ...
                    fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}`)
                        ...
                        .then(data => {
                            if (data && data.display_name) {
                                ...
                                document.getElementById('alamatTerpilihInput').value = data
                                    .display_name;
                                document.getElementById('btnPilihLokasi').disabled = false;
                            } else {
                                document.getElementById('alamatTerpilih').innerText =
            ...
            if (btnPilihLokasi) {
                btnPilihLokasi.onclick = function(e) {
                    ...
                    if (document.getElementById('alamatTerpilihInput').value && document.getElementById(
                            'latitudeInput').value && document.getElementById('longitudeInput').value) {
            ...
            let editLeafletMap, editLastCenter = {
                lat: -7.977,
                lng: 112.634
            };
            ...
            function updateEditAddress(lat, lng) {
                ...
                editReverseGeocodeTimeout = setTimeout(() => {
                    fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}`)
                        ...
                        .then(data => {
                            if (data && data.display_name) {
                                document.getElementById('editAlamatTerpilih').innerText = data
                                    .display_name;
                                document.getElementById('editAlamatTerpilihInput').value = data
                                    .display_name;
                                document.getElementById('editBtnPilihLokasi').disabled = false;
                            } else {
                                document.getElementById('editAlamatTerpilih').innerText =
                                    'Alamat tidak ditemukan';
                                document.getElementById('editAlamatTerpilihInput').value = '';
                                document.getElementById('editBtnPilihLokasi').disabled = true;
                            }
            ...
            if (editBtnPilihLokasi) {
                editBtnPilihLokasi.onclick = function(e) {
                    e.preventDefault();
                    if (document.getElementById('editAlamatTerpilihInput').value && document.getElementById(
                            'editLatitudeInput').value && document.getElementById('editLongitudeInput').value) {
