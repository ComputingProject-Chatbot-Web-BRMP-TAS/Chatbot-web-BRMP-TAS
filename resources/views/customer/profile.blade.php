...
@section('content')
    <style>
        ...
        .profile-info-action:focus,
        .profile-info-action:active {
            text-decoration: none;
        }
    ...
    <div class="profile-tabs">
        ...
        <a href="{{ route('addresses') }}" class="profile-tab @if (Route::currentRouteName() == 'addresses') active @endif">Daftar
            Alamat</a>
    </div>
    <div class="profile-main">
        <div class="profile-left">
            ...
            <form method="POST" action="{{ route('profile.upload_foto') }}" enctype="multipart/form-data"
                style="width:100%;">
                @csrf
                <input type="file" name="profile_picture" id="fotoProfilInput" accept="image/jpeg,image/png"
                    style="display:none;" onchange="this.form.submit()">
                <button type="button" class="btn-green w-100"
                    onclick="document.getElementById('fotoProfilInput').click()">Pilih
                    Foto</button>
            </form>
            @if ($errors->has('profile_picture'))
                <div
                    style="color: #d32f2f; background: #fff3f3; border: 1px solid #f8d7da; padding: 10px 16px; border-radius: 8px; margin-bottom: 16px; text-align:center;">
                    <strong>Gagal upload foto:</strong> {{ $errors->first('profile_picture') }}
                </div>
            @endif
            @if (session('success'))
                <div
                    style="color: #388e3c; background: #eaffea; border: 1px solid #b2dfdb; padding: 10px 16px; border-radius: 8px; margin-bottom: 16px; text-align:center;">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="profile-actions">
                <button type="button" class="btn-green w-100" style="background: #fff;color:#3C9A40" data-bs-toggle="modal"
                    data-bs-target="#modalGantiPassword">Ganti Kata Sandi</button>
            </div>
        </div>
        <div class="profile-right">
            ...
            <div class="profile-info-row">
                ...
                @else
                    <div class="profile-info-action" style="color:#4CAF50; cursor:pointer;" data-bs-toggle="modal"
                        data-bs-target="#modalEditTanggalLahir">Tambah Tanggal Lahir</div>
                @endif
            </div>
            <div class="profile-info-row">
                ...
                @else
                    <div class="profile-info-action" style="color:#4CAF50; cursor:pointer;" data-bs-toggle="modal"
                        data-bs-target="#modalEditJenisKelamin">Tambah Jenis Kelamin</div>
                @endif
            </div>
            ...
            <div class="profile-info-row">
                ...
                <div style="display: flex; align-items: center;flex-wrap:wrap;">
                    ...
                    <div class="profile-info-action py-2" data-bs-toggle="modal" data-bs-target="#modalEditEmail">Ubah</div>
                </div>
            </div>
            <div class="profile-info-row">
                ...
                @if (Auth::user()->phone ?? false)
                    <div style="display: flex;flex-wrap: wrap;">
                        <div style="display: flex; align-items: center;margin-right: 10px;flex-wrap:wrap;">
                            <div class="profile-info-value">{{ Auth::user()->phone }}
                            </div>
                            @if (!Auth::user()->isPhoneVerified())
                                <div class="profile-info-badge">Belum diverifikasi</div>
                            @endif
                        </div>
                        <div style="display: flex; align-items: center;flex-wrap:wrap;">
                            <span class="profile-info-action align-content-center py-2" data-bs-toggle="modal"
                                data-bs-target="#modalEditPhone">Ubah</span>
                            @if (!Auth::user()->isPhoneVerified())
                                <button class="btn-green p-1" id="btnVerifikasiPhone" data-bs-toggle="modal"
                                    data-bs-target="#modalVerifikasiPhone">Verifikasi</button>
                            @endif
                        </div>
                    </div>
                @else
                    <div class="profile-info-value" style="color:#4CAF50; cursor:pointer;" data-bs-toggle="modal"
                        data-bs-target="#modalEditPhone">Tambah Nomor HP</div>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-2" style="color:#666;">Kamu hanya dapat mengubah nama 1 kali lagi. Pastikan nama sudah
                        benar.</div>
    ...
    <div class="modal fade" id="modalEditTanggalLahir" tabindex="-1" aria-labelledby="modalEditTanggalLahirLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="mb-2" style="color:#666;">Kamu hanya dapat mengatur tanggal lahir satu kali. Pastikan
                        tanggal lahir sudah benar.</div>
                    <form method="POST" action="{{ route('profile.update') }}">
                        @csrf
                        <div class="mb-3">
    ...
    <div class="modal fade" id="modalEditJenisKelamin" tabindex="-1" aria-labelledby="modalEditJenisKelaminLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="mb-2" style="color:#666;">Kamu hanya dapat mengubah data jenis kelamin 1 kali lagi.
                        Pastikan data sudah benar</div>
                    <form method="POST" action="{{ route('profile.update') }}">
                        @csrf
                        <div class="d-flex justify-content-center gap-4 mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="gender" id="genderPria"
                                    value="Pria" checked>
    ...
    <!-- Modal Edit Nomor HP -->
    <div class="modal fade" id="modalEditPhone" tabindex="-1" aria-labelledby="modalEditPhoneLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="mb-2" style="color:#666;">Pastikan nomor HP Anda aktif untuk keamanan dan kemudahan
                        transaksi</div>
                    <form method="POST" action="{{ route('profile.update') }}">
                        @csrf
                        <div class="mb-3">
                            ...
                            <input type="tel" class="form-control" id="inputPhone" name="phone"
                                pattern="^(08|628)[0-9]{7,11}$" placeholder="Contoh: 08123456789"
                                value="{{ Auth::user()->phone }}">
                            <div class="form-text">Kami akan mengirimkan kode verifikasi melalui SMS</div>
                        </div>
                        <button type="submit" class="btn-green w-100">Selanjutnya</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalVerifikasiPhone" tabindex="-1" aria-labelledby="modalVerifikasiPhoneLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form method="POST" action="{{ route('profile.verify_phone') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="otp_code" name="otp_code" maxlength="6"
                                required placeholder="Kode OTP">
                            <div class="form-text">Kode OTP telah dikirim ke nomor HP Anda.</div>
                            <button type="button" class="btn btn-link p-0" id="btnKirimUlangOTP">Kirim Ulang
                                OTP</button>
                            <div id="otpStatus" class="mt-2" style="font-size:0.95em;"></div>
                        </div>
                        <button type="submit" class="btn-green w-100">Verifikasi</button>
                    </form>
                </div>
            </div>
        </div>
